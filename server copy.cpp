...
struct Conn {
    ...
    struct Buffer *incoming = (struct Buffer *)malloc(sizeof(struct Buffer));
    struct Buffer *outgoing = (struct Buffer *)malloc(sizeof(struct Buffer));
};
...
struct Buffer {
    ...
    // Function to get pointer to the data
    uint8_t *(*data)(struct Buffer *self);
    size_t (*size)(struct Buffer *self);
};
...
static void buf_append(struct Buffer *buf, const uint8_t *data, size_t len) {
    ...
    size_t space_at_end = buf->buffer_end - buf->data_end;
    size_t space_at_start = buf->data_begin - buf->buffer_begin;
    if (space_at_end < len) {
        if (space_at_start + space_at_end >= len) {
            // shift data to beginning of buffer
            memmove(buf->buffer_begin, buf->data_begin, buf->data_end - buf->data_begin);

            // adjust pointers to data end and begin
            buf->data_end -= buf->data_begin - buf->buffer_begin;
            buf->data_begin = buf->buffer_begin;
        } else {
            // reallocate the buffer
            ssize_t curr_size = buf->buffer_end - buf->buffer_begin;
            ssize_t new_size = curr_size * 2 > curr_size + len ? curr_size * 2 : curr_size + len;
            ...
            ssize_t data_size = buf->data_end - buf->data_begin;
...
static void buf_consume(struct Buffer *buf, size_t len) {
